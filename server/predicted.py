...
class Predictor:
    ...
    df_dnn = pd.read_csv('2017-07-01_2018-03-18.csv').drop(
        ['Date and time', 'Month', 'Weekday'], axis=1).iloc[:, 1:4].values
    ...
    def future_weather(self, api, lat, lon, hours, time):
        ...
        for i in range(days):
            ...
            data.extend([self.ftoc(hour.temperature)
    ...
    def predict_future_lstm(self, predict_hours, model_file, inputs, weather, inputs_steps):
        ...
        predicted_energy_usage = np.expand_dims(
            np.asarray(predicted_energy_usage), axis=1)
        predicted_energy_usage_tbl = np.zeros(
            shape=(len(predicted_energy_usage), 4))
        predicted_energy_usage_tbl[:, 0] = predicted_energy_usage[:, 0]
        predicted_energy_usage = sc.inverse_transform(
            predicted_energy_usage_tbl)[:, 0]
    ...
    def plot_data(self, predictions, weather_data, model):
        ...
        ax.plot(weather_data, color='green', label='temperature',
                linestyle='dashed', alpha=0.2)
        avg_consumption = round(
            sum([sum(i) for i in zip(*predictions)])/float(len(model)), 2)
