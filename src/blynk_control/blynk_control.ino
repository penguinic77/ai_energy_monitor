...
#define INFLUXDB_URL "https://nic.connectivity.tw"
...
/*blynk相關設定*/
#define BLYNK_TEMPLATE_ID           "TMPLYgvJKiy0"
#define BLYNK_DEVICE_NAME           "Energy Monitor"
#define BLYNK_AUTH_TOKEN            "q-dDVq9x915bZDePTg4ZC2YDzbeIYonf"
...
void check_power()
{
  ...
  watt=mA*110/1000;
  if(watt<minv)
  {watt=random(300.0,500.0)/100.0;}
  else if (watt>maxv)
  {watt=random(50.0,58.0);}
  watt_average+=watt;
  if (count ==60)
  {
...
void check_prediction()
{
 ...
 pvalue_kWh=pvalue/60000;//轉成度數
 Blynk.virtualWrite(V1, pvalue_kWh);
 money_cost();
 timer.disable(timer_prediction);
}
//向server發送預測指令
void get_predict(){
  String serverPath = "https://nicserver.connectivity.tw";
